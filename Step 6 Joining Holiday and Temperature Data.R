...
### ------- Step 6: Joining Holiday and Temperature Data -------- ###
...
temp <- temp %>% 
  ...
  rename(AvgTempF = Average.Temperature...F.,
...
readydata <- subset(temp, select = -c(Voltage, 
                                      
                                      SeasonName:Date.y))



## Save the ready data set

saveRDS(readydata, file = "EnergyDataHolidaysAndTemp.rds")